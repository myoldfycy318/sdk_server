...
public class WxJsApiPayForwardPayService implements ForwardPayService {
    ...
    public SdkOauthResult toPayPage(H5GameEntity h5GameEntity, AppInfoEntity appInfoEntity) throws Exception {
        ...
        try {
            ...
            //构建微信公众号支付JsApi
            result = bulidWxJsPayPage(h5GameEntity, appInfoEntity, orderEntity);
            if (!result.isSuccess())
                return result;
            ...
            String wxJsApiParams = (String) result.getData();
     ...
     * @throws Exception
     */
    private SdkOauthResult bulidWxJsPayPage(H5GameEntity h5GameEntity, AppInfoEntity appInfoEntity, OrderEntity orderEntity) throws Exception {
        ...
        String wxPublicAppId2MchId = payConfig.getString("wx.public2mch."+wxPublicAppId);
        String wxPublicMchApiKey = payConfig.getString("wx.mch.apikey."+wxPublicAppId2MchId);
        if (StringUtils.isBlank(wxPublicAppId) || StringUtils.isBlank(wxPublicAppId2MchId) || StringUtils.isBlank(wxPublicMchApiKey)) {
            logger.info("微信公众号支付商户信息配置错误，公众号id:{},商户id：{},商户签名apikey:{}",wxPublicAppId,wxPublicAppId2MchId,wxPublicMchApiKey);
            return SdkOauthResult.failed("微信支付商户信息配置错误");
        }
        ...
        WeChatEntity weChatEntity = new WeChatEntity.Builder().openId(h5GameEntity.getOpenId()).
                ...
                appId(wxPublicAppId).mchId(wxPublicAppId2MchId).signKey(wxPublicMchApiKey).build();
        ...
        return SdkOauthResult.success(sb.toString().substring(1));
    }
    ...
    private OrderEntity getOrderEntity(H5GameEntity h5GameEntity, AppInfoEntity appInfoEntity, Double price) {
        ...
        String gameOrderNo = StringUtils.isBlank(h5GameEntity.getOutOrderNo()) ? GenOrderCode.genOutOrderNo("bq_") : h5GameEntity.getOutOrderNo();
