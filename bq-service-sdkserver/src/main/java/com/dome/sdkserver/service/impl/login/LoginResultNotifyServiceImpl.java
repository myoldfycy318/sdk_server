...
public class LoginResultNotifyServiceImpl implements LoginResultNotifyService {
    ...
    @Override
    public void startNotify(String userId, String key, String clientId, boolean isAdult, String notifyUrl) {
        executeStartNotifyThread(userId, key, clientId, isAdult, notifyUrl);
    }
     ...
     * @return 通知是否成功
     */
    private boolean sendAsyncNotifyRequest(String userId, String key, String clientId, boolean isAdult, String notifyUrl) {
        List<NameValuePair> paramList = new ArrayList<NameValuePair>();

        paramList.add(new BasicNameValuePair("loginNo", key));
        paramList.add(new BasicNameValuePair("userId", String.valueOf(userId)));
        paramList.add(new BasicNameValuePair("appCode", clientId));
        paramList.add(new BasicNameValuePair("isAdult", String.valueOf(isAdult)));
        ...
        try {
            ...
            log.error("发送异步通知失败(Exception)!userId=" + userId + ",loginNo=" + key + ",clientId=" + clientId + ",isAdult=" + isAdult, e);
            isSuccess = false;
        }
     ...
     * @param isAdult   沉迷信息
     */
    private void executeStartNotifyThread(final String userId, final String key, final String clientId, final boolean isAdult,
                                          final String notifyUrl) {
        NotifyExecutor.executePayResultNotify(new Runnable() {
            @Override
            public void run() {
                try {
                    sendAsyncNotifyRequest(userId, key, clientId, isAdult, notifyUrl);
                } catch (Exception e) {
                    log.error("登录结果异步通知线程异常! userId=" + userId + ",key=" + key + ",clientId=" + clientId + ",notifyUrl="
                            + notifyUrl + ",isAdult=" + isAdult, e);
                }
            }
        });
    }
