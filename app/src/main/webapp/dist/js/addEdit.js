        ...
        var addEdit = Class.create(
            {
                ...
                getData: function (callback) {
                    var _this = this;
                    Util.ajaxData('merchantapp/toview', 'POST', {appCode: _this.param.appCode}, function (d, bool) {
                        if (bool) {
                            var ary = [];
                            ...
                            $('#tpl_con').append(html);
                            _this.gamesBind(0, d.data.appType, d.data.gameType);
                            callback();
                        }
                    });
                },
                ...
                /**
                 * @status 0应用类型  1游戏类型*/
                gamesBind: function (status, id, gameType) {
                    Util.ajaxData('apptypeattr/appTypeAttrList', 'GET', {typeAttrParentCode: id}, function (d, bool) {
                        if (bool) {
                            ...
                            if (status == 0) {
                                ...
                                if (d.data && d.data[0]) {
                                    if (!gameType) {
                                        gameType = d.data[0].typeAttrCode;
                                    }
                                    this.gamesBind(1, gameType);
                                }
                            } else {
                ...
                subBind: function () {
                    $('.btn_sub').on('click', function () {
                        ...
                        if (this.required != 1) {
                            Util.ajaxData('merchantapp/save', 'POST', data, function (d, bool) {
                                if (bool) {
                ...
                formVal: function () {
                    ...
                    } else if ($('.app_gameType').val() == '30000000') {
                    ...
                    $('input:text').each(function () {
                        if ($('.app_gameType').val() == '10000000') {
                            ...
                            if ($(this).attr('data-name') != "gameDesc") {
                                if ($(this).hasClass('required')) {
                                    if ($(this).val() == '') {
                                        _this.required = 1;
                                        if (!$(this).parents('.boxInput').hasClass('errorTips')) {
                ...
                imgRemove: function () {
                    ...
                    $('.btn-del').on('click', function () {
                        _this.imgDelete($(this));
                    });
                },
                imgDelete: function ($this) {
                    ...
                    if($this.parents('.img_box').find('.edit-img').length <= 9){
                        $this.parents('.fileBox').find('.webuploader-container').show();
                    }
