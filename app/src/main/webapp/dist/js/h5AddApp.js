...
define('entries/util/upload',[
  'jquery', 
  'template', 
  'lib/lodash/dist/lodash.min',
  './util',
  'fileUpload'
], function (
  $,
  ...
  var upload = function (item, uploadSuccessCB) {
    var _this = this;
  ...
  'entries/service/h5AddApp',[
    ...
    var app = Class.create({
      setOptions: function (opts) {
        ...
        init: function (opts) {
          var appCode = Util.getQueryString('appCode'),
              isShow = Util.getQueryString('show') === 'true'
              ;
          ...
          console.log('init webgameAddApp');

          if (appCode) {
            this.getItem(appCode, this.isShow);
          } else {
            this.renderForm({});
          }
        },
        ...
        preHandleItem: function(item) {
          var channle = 'h5game';
          if (/\HD/.test(item.appCode)) {
            channle = 'h5game-d';
          }
        ...
        bindSubmitBtn: function () {
          $('.btn_sub').on('click', function () {
            ...
            cb = function (d, bool) {
              ...
              if (bool) {
                Util.tipsAlert(null, '应用' + msg + '成功', 'tips_success');
                setTimeout(function () {
            ...
            merchantAppAPI.save(data.data, cb);
        ...
        fileLoad: function (item) {
          ...
          uploadSuccessCB = function (file, response) {
        ...
        getItem: function(appCode, isShow) {
          var query = {
            appCode: appCode
          }, 
          data = {},
          cb = function(d, bool) {
            if (bool) {
              data = this.preHandleItem(d.data);
              this.renderForm($.extend({}, data, {isShow: isShow}));
            }
          }.bind(this);

          merchantAppAPI.fetchView(query, cb);
        },
        ...
        renderForm: function(data) {
          var resultTpl = template(this.formTpl, data);

          $('#formPanel').append(resultTpl);
        ...
        formHandle: function () {
          var items = [
              'appName', 'appIcon', 'appDesc', 
              't', 'appUrl', 'copyRightFlag', 'compareSoftwareUrl',
              'gameType', 'developers' 
            ],
            selectorPrefix = '.fromBox .'
            ;
...
require(
  ['jquery','entries/util/base','entries/service/h5AddApp'],
  function ($,Base, App) {
    $(function () {
      Base.init(function () {
        new App();
...
define("entries/action/h5AddApp.js", function(){});
