...
define(
    ...
    function ($,
              ...
              Util,
              template,
              merchantAppAPI, pages, WebUploader) {
        var app = Class.create({
            ...
            bookDetail: function () {
                Util.ajaxData('merchant/toview', 'GET', {}, function (d, bool) {
                    if (bool) {
            ...
            getTabelData: function (obj, status) {
                if (obj) {
                ...
                Util.ajaxData('merchantapp/v1/applist', 'GET', obj, function (d, bool) {
                    if (bool) {
            ...
            editAppStatus: function (obj) {
                ...
                obj.data.appCode = this.appCode;
                obj.data.status = this.apply_status;
                Util.ajaxData('merchantapp/updateState', 'POST', obj.data, function (d, bool) {
                    if (bool) {
            ...
            callbackUpdate: function (obj) {
                ...
                obj.data.appCode = this.appCode;
                merchantAppAPI.applyModifyCallbackUrl(obj.data, function (d, bool) {
                    if (bool) {
                        window.location.reload();
                    } else {
                        Util.hideAlertBox();
            ...
            delApp: function (appCode) {
                Util.ajaxData('merchantapp/dodel', 'GET', {appCode: appCode, status: status}, function (d, bool) {
                    if (bool) {
                        window.location.reload();
                    }
                });
            ...
            applyJr: function () {
                ...
                $('.apply_jr').on('click', function () {
                    var id = _this.apply_status = $(this).attr('data-value');
                    var html, el = '.alert_box';
                    _this.appCode = $(this).parents('tr').find('td').eq(0).text();
                    if (id == "11") {
                        html = template('joinupTpl', {});
                        $('.alert-Box').css('overflow-y','hidden');
                        $(el + ' .alert-Box').html('').html(html);
                        $(el).show();
                        _this.bindJoinUpEvent();
                ...
                $('.update_callback').on('click', function () {
                    ...
                    cb = function (d, bool) {
                        if (bool) {
                            html = template('joinupTpl', {
                                update: true,
                                testRegistCallBackUrl: d.data.testRegistCallBackUrl,
                                loginCallBackUrl: d.data.loginCallbackUrl,
                                testLoginCallBackUrl: d.data.testLoginCallbackUrl,
                                registCallBackUrl: d.data.registCallBackUrl,
                                testPayCallBackUrl: d.data.testPayCallbackUrl,
                                payCallBackUrl: d.data.payCallbackUrl,
                                appIcon: d.data.appIcon
                            });
                            $('.alert-Box').css('overflow-y','scroll');
            ...
            applyBind: function () {
                $(".app_check_box").on('click', function () {
                    var className = $(this).attr('data-class');
                    if (this.checked) {
                        var parent = $('.' + className).show().parents('.alert-Box');
                        parent.height(parent.height() + 150);
                    } else {
                        var parent = $('.' + className).hide().parents('.alert-Box');
                        parent.height(parent.height() - 140);
                    }
                });
            },
            applySub: function () {
                var obj = {}, _this = this;
                $('.alertContent input').blur(function () {
                    _this.applySubForm($(this));
                });
                $('.sub_btn').on('click', function () {
                    obj = _this.applySubForm($(this));
                    if (obj.required) {
                        Util.hideAlertBox();
                        _this.editAppStatus(obj);
                    }
                });
            ...
            applySubForm: function ($this) {
                var obj = {
                    ...
                    if ($this.val() != '') {
                        ...
                        if (Util.requireURL($this.val())) {
                            $this.parent().removeClass('errorTips');
                            obj.data[$this.attr('data-name')] = $this.val();
                        } else {
                            $this.parent().addClass('errorTips');
                            obj.required = false;
                        }
                    } else {
                ...
                $('.alertContent').find(':checked').each(function () {
                    if ($(this).parent().index() == 1) {
                        $this.parents('.alert-Box').find("input:text").each(function (i) {
                            if (i < 2) {
                                getVal($(this));
                            }
                        });
 ...
 */
require(
    ['jquery','entries/util/base','entries/service/appList'],
    function ($,Base, AppList) {
        $(function () {
            Base.init(function () {
                new AppList();
            })
...
define("entries/action/appList.js", function(){});
