...
/**
 * Created by leilihuang on 16/8/9.
 */
define(
    'entries/service/channelmanage-application',['jquery', 'Class', 'entries/util/util', 'fileUpload', 'iCheck'],
    function ($, Class, Util, WebUploader) {
        var index = Class.create(
            {
                ...
                agreementBind: function () {
                    var _this = this;
                    $('input.icheckbox_square').on('click', function () {
                        if (this.checked) {
                            //$(this).attr('checked',true);
                            _this.agree_checked = true;
                            $('.btn_sub').addClass('sub_cur');
                        } else {
                            _this.agree_checked = false;
                            //$(this).attr('checked',false);
                            $('.btn_sub').removeClass('sub_cur');
                        }
                    });
                },
                submitBind: function () {
                    ...
                    $('.btn_sub').on('click', function () {
                        if (!$(this).hasClass('sub_cur')) {
                            return false;
                        }

                        var data = _this.formVal();
                        if (_this.required != 0 && _this.agree_checked) {
                            _this.required = 0;
                            return false;
                        } else {
                            data.channelCode = _this.params.channelCode;
                            Util.ajaxData('channel/save/1', 'POST', data, function (d, bool) {
                                if (bool) {
                                    window.location.href = 'channelmanage-reviewing.html';
                                }
                            });
                        }
                    });
                ...
                formVal: function () {
                    ...
                    $('input:text').each(function () {
                        if ($(this).hasClass('required')) {
                            if ($(this).val() == '') {
                                ...
                                if($(this).attr('data-name') == 'name' || $(this).attr('data-name') == 'companyName'){
                                ...
                                if($(this).attr('data-name')=='companyUrl'){
                    ...
                    $('input:hidden').each(function () {
                        if ($(this).hasClass('required')) {
                            if ($(this).val() == '') {
                                _this.required = 1;
                                if (!$(this).next().hasClass('borRed')) {
                                    $(this).next().addClass('borRed');
                                }
                            } else {
                                $(this).next().removeClass('borRed');
                            }
                        }
                        obj[$(this).attr('data-name')] = $(this).val();
                    });
                ...
                fileLoad: function (item) {
                    ...
                    uploader.on('uploadSuccess', function (file, response) {
                        ...
                        if (response.responseCode == 1000) {
                            $(item).addClass("cur").addClass('btn_file').find('.webuploader-pick').text('证件已传');
                            $(item).parents('.boxInput').find('.legalImageUrl').show().find('img').attr('src', response.data.imgUrl);
                            $(item).parents('.boxInput').find(".imgUrl").val(response.data.imgUrl);
                            $(item).removeClass('borRed');
                ...
                delImg: function () {
                    $('.file_del').on('click', function () {
                        var btn_file = $(this).parents('.boxInput').find('.btn_file');
                        $(this).parents('.legalImageUrl').hide().find('img').attr('src', '');
                        btn_file.removeClass('cur').find('.webuploader-pick').text('证件上传');
                        btn_file.prev().val('');
                    });
                },
                userXy: function () {
                    Util.agreedBind($('.btn_sub'));
                }
            });
...
require(
    ['jquery','entries/util/base','entries/service/channelmanage-application'],
...
define("entries/action/channelmanage-subapply.js", function(){});
