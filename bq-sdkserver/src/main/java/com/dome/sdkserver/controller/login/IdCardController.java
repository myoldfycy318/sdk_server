    ...
    public SdkOauthResult realNameAuth(HttpServletRequest request, User user) {
        ...
        try {
            ...
            if (idCardService.isNRealName()) {
                ...
                boolean isAdult = isAdult(user.getIdCardNo());//沉迷
                if (StringUtils.isNotBlank(loginNotifyUrl)) {
                    if (StringUtils.isNotBlank(user.getUserId())) {
                        loginResultNotifyService.startNotify(user.getUserId(), user.getKey(), request.getParameter("appCode"), isAdult, loginNotifyUrl);
                    } else {
