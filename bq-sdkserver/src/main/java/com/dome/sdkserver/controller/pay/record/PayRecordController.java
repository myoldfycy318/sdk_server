...
public class PayRecordController extends PayBaseController {
    ...
    @RequestMapping("/list")
    @ResponseBody
    public SdkOauthResult payList(HttpServletRequest request, PayRecordEntity orderEntity) {
        log.info("充值记录查询用户请求参数：{}", JSONObject.toJSONString(orderEntity));
        Map<String, Object> resultMap = new HashMap<String, Object>(2);
        try {
            if (StringUtils.isBlank(orderEntity.getBuyerId()) || !PayOriginEnum.isInsidePay(orderEntity.getPayOrigin())) {
                return SdkOauthResult.failed("比传参数错误");
            }
            if (orderEntity.getCreateTime() == null) return SdkOauthResult.failed("开始时间不能为空");
            if (orderEntity.getFinishTime() == null) return SdkOauthResult.failed("结束时间不能为空");
            if (orderEntity.getCreateTime().after(orderEntity.getFinishTime()))
                return SdkOauthResult.failed("开始时间要在结束时间之前");
            if (orderEntity.getFinishTime().before(DateUtil.getDate("2017-10-31 23:59:59", "yyyy-MM-dd HH:mm:ss"))) {
                resultMap.put("totalCount", 0);
                resultMap.put("list", Collections.EMPTY_LIST);
                return SdkOauthResult.success(resultMap);
            }
            if (orderEntity.getCreateTime().before(DateUtil.getDate("2017-10-31 23:59:59", "yyyy-MM-dd HH:mm:ss"))) {
                orderEntity.setCreateTime(DateUtil.getDate("2017-11-01 00:00:00", "yyyy-MM-dd HH:mm:ss"));
            }
            PageDto pageDto = new PageDto();
            //封装请求参数
            encapsulatePageParams(request, pageDto);
            //支付月
            orderEntity.setOrderMonths(DateUtils.getMonthBetween(DateFormatUtils.format(orderEntity.getCreateTime(), "yyyyMM"), DateFormatUtils.format(orderEntity.getFinishTime(), "yyyyMM")));
            List<PayRecordEntity> list = payRecordService.queryPayStream(orderEntity, pageDto);
            resultMap.put("totalCount", pageDto.getTotalCount());
            resultMap.put("list", list);
            return SdkOauthResult.success(resultMap);
        } catch (Exception e) {
