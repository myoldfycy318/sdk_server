...
/**
 * util/api/chargepointAPI
 */

define('entries/util/api/chargepointAPI',['entries/util/util'], function(Util) {
  var fetchPointList = function(query, cb) {
    Util.ajaxData('chargepoint/toList', 'GET', query, cb);
  }

  var insert = function(data, cb) {
    Util.ajaxData('chargepoint/add', 'POST', data, cb);
  }

  var update = function(data, cb) {
    Util.ajaxData('chargepoint/update', 'POST', data, cb);
  }

  var change = function(data, cb) {
    Util.ajaxData('chargepoint/change', 'POST', data, cb);
  }

  var fetchItem = function(query, cb) {
    Util.ajaxData('chargepoint/toview', 'POST', query, cb);
  }

  var deleteItem = function(query, cb) {
    Util.ajaxData('chargepoint/delete', 'POST', query, cb);
  }

  var batchPost = function(query, cb) {
    Util.ajaxData('chargepoint/batchPost', 'POST', query, cb);
  }
...
define('entries/util/formHandle',[
  'jquery',
  './util'
], function (
  $,
  util
    ...
    var formHandle = function (items, selectorPrefix, errorTip) {
      var result = {
          data: { },
          canSubmit: true
        },
      ...
      if (!errorTip) {
        errorTip = '<span class="error red">必填项</span>'
      }

      for (i; i < items.length; i++) {
        el = $(selectorPrefix + items[i]);
        val = $(el).val()
        ;
        ...
        valid = $(el).attr('data-valid');

        if ($(el).hasClass('required')) {
          if (!val) {
            appendErrorTip(el, parentEL, errorTip);

            addErrorClass(el, parentEL, errorClass);
            
            result.canSubmit = false;
            
          } else {
            $(el).parents(parentEL).removeClass(errorClass);
          }
        }
        ...
        if (val) {
          if (valid) {
            j = 0;
            valids = valid.split(/\s+/);
            for (j; j < valids.length; j++) {
              switch (valids[j]) {
                case 'http':
                  if (!util.http(val)) {
                    result.canSubmit = handleValidTip(el, parentEL, errorTip, errorClass); 
                  }
                  break;
                case 'digit':
                  if (!util.digit(val)) {
    ...
    var appendErrorTip = function(el, parentEL, errorTip) {
      if ($(el).parents(parentEL).find('.error').length === 0) {
        $(el).parents(parentEL).append(errorTip);
      }
    }

    var addErrorClass = function(el, parentEL, errorClass) {
      if (!$(el).parents(parentEL).hasClass(errorClass)) {
        $(el).parents(parentEL).addClass(errorClass);
      }
    }

    var handleValidTip = function(el, parentEL, errorTip, errorClass) {
      // result.canSubmit = false;
      appendErrorTip(el, parentEL, errorTip);
      $(el).parents(parentEL).find('.error').html($(el).attr('placeholder'));
      addErrorClass(el, parentEL, errorClass);
      return false;
    }
...
define('entries/service/webgamePoint',[
  'jquery',
  'Class',
  'entries/util/util',
  'template',
  'entries/util/api/chargepointAPI',
  'entries/util/formHandle',
  'pages'
], function (
  $,
  Class,
  ...
  formHandle
    ...
    var app = Class.create({
      setOptions: function (opts) {
        ...
        init: function (opts) {
          this.setOptions($.extend({}, opts, {
            param: Poss.getUrl()
          }));

          console.log('init webgamePoint');
        ...
        addPoint: function () {
          $('.add_point').on('click', function () {
            ...
            result = template(this.els.pointTpl, {
              status: 0,
              appCode: this.param.appCode
            });

            $(result)
              .find('.clone').on('click', this.closeAlert).end()
              .find('#submitBtn').on('click', this.insertOrUpdatePoint.bind(this, 'submit')).end()
              .find('#saveBtn').on('click', this.insertOrUpdatePoint.bind(this)).end()
              .appendTo('body');
        ...
        batchBtn: function () {
          $('#batchSub').on('click', function () {
            var data = {
              appCode: this.param.appCode,
              chargePointCodes: []
            };

            $('#itemContainer').find('input:checked').each(function () {
              data.chargePointCodes.push($(this).attr('data-chargePointCode'));
            });

            data.chargePointCodes = data.chargePointCodes.join(',');

            if (data.chargePointCodes == '') {
              Util.tipsAlert(null, '请选择你要提交的计费点信息', 'tips_error');
            } else {
              this.submitPoint(data);
            }
          }.bind(this));
        },
        ...
        editPoint: function () {
          var that = this;
          $('.edit_point_btn').on('click', function () {
            var result, chargePointCode, query, cb
              ;

            query = {
              appCode: that.param.appCode,
              chargePointCode: $(this).attr('data-chargePointCode')
            }
            ...
            cb = function (d, bool) {
              ...
              if (item.status === 40 || item.status === 30) {
                type = 'update'
              }

              result = template(that.els.pointTpl, item);

              $(result)
                .find('.clone').on('click', that.closeAlert).end()
                .find('.sub_btn').on('click', that.insertOrUpdatePoint.bind(that, type)).end()
                .appendTo('body');
            }

            chargepointAPI.fetchItem(query, cb);

          });
        },

        bindDeletePoint: function () {
          var that = this;
          $('.del_point_btn').on('click', function () {
            var query = {
              appCode: that.param.appCode,
              chargePointCode: $(this).attr('data-chargePointCode')
            }

            Util.tipsAlert(null, '确认删除？', 'tips_warning_alert', function () {
              that.deletePoint(query);
            });
          });
        },
        ...
        bindSubmitPoint: function () {
          var that = this;
          $('.sub_point_btn').on('click', function () {
            var query = {
              appCode: that.param.appCode,
              chargePointCodes: $(this).attr('data-chargePointCode')
            };

            that.submitPoint(query);
        ...
        bindViewPoint: function () {
          var that = this;
          $('.view_point_btn').on('click', function () {
            var query = {
              appCode: that.param.appCode,
              chargePointCode: $(this).attr('data-chargePointCode')
            };
        ...
        bindChangeEvent: function () {
          var that = this;
          $('.bg_point_btn').on('click', function () {
            var query = {
              appCode: that.param.appCode,
              chargePointCode: $(this).attr('data-chargePointCode')
            };

            that.changePoing(query);
          });
        },
        
        anClick: function () {
          $(".an_click").on("click", function () {
            $(this).toggleClass("an_cur").parents("tr").next().toggle();
          });
        },

        onKeyBind: function () {
          $('.util-digVail').keyup(function () {
            var bool = Util.digVail($(this).val());
            $(this).val(bool);
          });
        },

        appId: function () {
          $('#detail_box').find('.lab_con').text(Poss.getUrl().appCode);
          $('#detail_name').find('.lab_con').text(Poss.getUrl().appName);
        },

        pageBind: function () {
          Util.pageBind(this.total, this.pageSize, function (i) {
            this.pageNum = i + 1;
            if (this.initPage == 1) {
              this.getTable();
            }
        ...
        buildTable: function (obj, status) {
          var cb
            ;
          ...
          chargepointAPI.fetchPointList(obj, cb);
        },

        buildPoint: function (data, status, chargePointCode) {
          var _this = this;
          if (status == 0 || status == 2) {
            data.status = status;
            var html = template('tpl_edit_point', data);
            $(html).find('.sub_btn').on('click', function () {
              $(this).parents('.edit_point').hide();
              Util.showLoding();
              if (status == 0) {
                _this.addPointData('chargepoint/update', $('.edit_point'), chargePointCode, 10);
              } else {
                _this.addPointData('chargepoint/change', $('.edit_point'), chargePointCode);
              }
            }).end().find('.clone').on('click', function () {
              $(this).parents('.alert_box').remove();
            }).end().find('.util-digVail').keyup(function () {
              var v = Util.digVail($(this).val());
              $(this).val(v);
            }).end().appendTo('body');
          } else {
            var html = template('tpl_ck_point', data);
            $(html).find('.clone').on('click', function () {
              $(this).parents('.alert_box').remove();
            }).end().appendTo('body');
          }
        ...
        tplPage: function (data) {
          var obj = {
            pageStart: Util.pageStart(this.pageNum, this.pageSize, data.totalCount),
            pageEnd: Util.pageEnd(data.chargeList.length, this.pageNum, this.pageSize),
            totalCount: data.totalCount
          };
          var html = template('tpl_table_page', obj);
          $('#pageCon').empty().append(html);
        },
        ...
        renderTable: function (data) {
          var html = template('tpl_table', data);
          this.tplPage(data);
          $('#tbody').empty().append(html);

          this.anClick();
          this.editPoint();
          this.bindDeletePoint();
          this.bindSubmitPoint();
          this.bindViewPoint();
        ...
        insertOrUpdatePoint: function (type) {
          var data,
            that = this,
            cb, msg, query
            ;

          data = this.formHandle(type);
          ...
          cb = function (d, bool) {
            if (bool) {
              Util.tipsAlert(null, '计费点' + msg + '成功', 'tips_success');
              setTimeout(function () {
                window.location.reload();
              }, Util.config.successDelay);
            }
          }
          ...
          switch (type) {
            case 'submit':
              ...
              chargepointAPI.insert(query, cb);
              break;
            case 'update':
              msg = '修改';
              query = $.extend({}, query, {
              ...
              chargepointAPI.update(query, cb);
              break;
            case 'change':
              msg = '变更';
              chargepointAPI.change(query, cb);
              break;
            default:
              chargepointAPI.insert(query, cb);
          }
        ...
        deletePoint: function (query) {
          var cb = function (d, bool) {
            if (bool) {
              Util.tipsAlert(null, '删除成功', 'tips_success');
              setTimeout(function () {
                window.location.reload();
              }, Util.config.deleteDelay);
            }
          };

          chargepointAPI.deleteItem(query, cb);
        },

        submitPoint: function (query) {
          var cb = function (d, bool) {
            if (bool) {
              Util.tipsAlert(null, '提交成功', 'tips_success');
              setTimeout(function () {
                window.location.reload();
              }, Util.config.successDelay);
            }
          ...
          Util.showLoding();
          chargepointAPI.batchPost(query, cb);
        },

        changePoing: function (query) {
          var cb = function (d, bool) {
            if (bool) {
              Util.tipsAlert(null, '变更计费点成功', 'tips_success');
              setTimeout(function () {
                window.location.reload();
              }, Util.config.successDelay);
            }
        ...
        formHandle: function (type) {
          var items = ['appCode', 'chargePointName', 'chargePointAmount', 'desc'],
            selectorPrefix = '.form-point .'
            ;

          if (type === 'update' || type === 'change') {
            items.push('chargePointCode');
          }

          return formHandle(items, selectorPrefix);
        },

        formVal: function () {
          var form = $('.form_box'), obj = {};
          form.find('input:text').each(function () {
            if ($(this).val()) {
              obj[$(this).attr('data-name')] = $(this).val();
            }
          });
          obj[form.find('select').attr('data-name')] = form.find('select').val();
          return obj;
        }
...
require(
  ['jquery','entries/util/base','entries/service/webgamePoint'], function ($,Base, App) {
    $(function () {
      Base.init(function () {
        new App();
      })
...
define("entries/action/webgamePoint.js", function(){});
