 ...
 */
define(
    'entries/service/management',['jquery', 'Class', 'entries/util/util', 'template', 'fileUpload'],
        ...
        var manage = Class.create(
            {
                ...
                folding: function () {
                  $('#down-fold').on('click', function () {
                      $(this).toggleClass('down_cur').parents('.bt_box').find('.down-folding').toggle();
                  });
                },
                getDate: function () {
                    var _this = this;
                    $('.contentBox .conBox .addHead .addCon .tit').append('AppCode : ' + _this.param.appCode);
                    $('.contentBox .conBox .addHead .addCon .tis').append('应用名称 : ' + _this.param.appName);

                    Util.ajaxData('pkgmanage/view','POST',{appCode: _this.param.appCode}, function (d,bool) {
                        if(bool){
                            var scHtml = template('tpl_sc_bt', d.data);
                            var xzHtml = template('tpl_xz_bt', d.data);
                            $('#sc_bt').empty().prepend(scHtml);
                            $('#xz_bt').append(xzHtml);
                            _this.changePackage();
                        }
                    },1);
                ...
                packageState: function () {
                    var _this = this;
                    Util.ajaxData('pkgmanage/canChange','POST',{appCode: _this.param.appCode}, function (d,bool) {
                        if(!bool){
                            $('.managebtn').html('上传包体');
                        }
                        this.changePackage();
                ...
                alertModel: function () {
                    var html = '<input class="file_input" type="file"/> <div class="file_btn">上传包体</div>';
                    Util.tipsAlert('变更包体', html, "manage_alert");
                },
                /**变更包体*/
                updateManage: function () {
                    var _this = this;
                    $(".manage_bgbt").on("click", function () {
                        _this.alertModel();
                    });
                },
                /**变更包体弹窗*/
                changePackage: function () {
                    $('.managebtn').on("click", function () {
                        $('#picker').show();
                        $('#file_change').show().find('.title').text($(this).text());
                        $('#thelist').empty();
                    });
                    $('.tips_success .suc_btn').on("click", function () {
                        $('#file_success').hide();
                    });
                    $('#file_error .error_btn').on("click", function () {
                        $('#file_error').hide();
                        $('#file_change').show();
                        $('#picker').show();
                    });
                    ...
                    $('.suc_complete').on("click", function () {
                ...
                fileLoad: function (item) {
                    ...
                    uploader = WebUploader.create({
                        ...
                        // 文件接收服务端。
                        server: Poss.getHost('pkgmanage/upload?appCode=' + this.param.appCode),
                        pick: '#picker',
                        fileVal: "pkg",
                    ...
                    uploader.on('uploadProgress', function (file, percentage) {
                        ...
                        var $li = $('#' + file.id),
                            ...
                            $text = $li.find('.progress .text');

                        // 避免重复创建
                        if (!$percent.length) {
                            $percent = $('<div class="statusBar"><div class="progress progress-striped active">' +
                                '<span class="percentage" role="progressbar" style="width: 0%"></span>' +
                                '<span class="text"></span>' +
                                '</div></div>').appendTo($li).find('.progress-bar');
                        }

                        $li.find('p.state').text('上传中');
                        $text.html(parseInt(percentage * 100) + '%');
                     ...
                     * 变更包体取消上传
                     */
                    $('#file_change').find('.clone').on('click', function () {
                        //$(this).parents('.alert_box').hide();
                        if(_file != null){
...
require(
    ['jquery','entries/util/base','entries/service/management'],
    function ($,Base, Management) {
        $(function () {
            Base.init(function () {
                new Management();
            });
...
define("entries/action/management.js", function(){});
