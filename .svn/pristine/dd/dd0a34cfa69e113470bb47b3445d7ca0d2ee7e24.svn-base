 ...
 */
define(
    'entries/service/point',['jquery', 'Class', 'entries/util/util', 'template', 'datetimepicker', 'pages'],
        ...
        var point = Class.create(
            {
                ...
                getTable: function (obj,status) {
                    if (!obj) {
                        obj = {};
                    }
                    obj.pageNum = this.pageNum;
                    obj.appCode = this.param.appCode;
                    Util.ajaxData('chargepoint/toList','POST',obj, function (d, bool) {
                        if(bool){
                            this.total = d.data.totalCount;
                            this.tplTable(d.data);
                            if(status){
                                this.pageBind();
                            }
                        }
                ...
                subPoint: function () {
                    ...
                    $('#subBtn').on('click', function () {
                        ...
                        _this.addPointData('chargepoint/add', $('.alert_box'),'', 10);
                    });

                    $('#saveBtn').on('click', function () {
                        $(this).parents('.alert_box').hide();
                        _this.addPointData('chargepoint/add', $('.alert_box'));
                    });
                },
                editPointBind: function () {
                    var _this = this, obj = {
                        appCode: this.param.appCode
                    };

                    var urls={
                        del:'chargepoint/delete',
                        bg:'chargepoint/change',
                        sub:'chargepoint/post',
                        edit:'chargepoint /update',
                        ck:'chargepoint/toview',
                        add:'chargepoint/add'
                    };

                    //编辑计费点
                    $('.edit_point_btn').on('click', function () {
                        obj.chargePointCode = $(this).parents('tr').find('td').eq(2).text();
                        _this.edtiPointData(obj, 0,obj.chargePointCode);
                    });

                    //查看计费点
                    $('.ck_point_btn').on('click', function () {
                        obj.chargePointCode = $(this).parents('tr').find('td').eq(2).text();
                        _this.edtiPointData(obj, 1);
                    });
                    ...
                    $('.sub_point_btn').on('click', function () {
                        Util.showLoding();
                        obj.chargePointCode = $(this).parents('tr').find('td').eq(2).text();
                        _this.deltePoint(obj,urls.sub);
                    });

                    //计费点变更
                    $('.bg_point_btn').on('click', function () {
                        obj.chargePointCode = $(this).parents('tr').find('td').eq(2).text();
                        _this.edtiPointData(obj, 2,obj.chargePointCode);
                    });
                },
                deltePoint: function (obj,url) {
                    Util.ajaxData(url,'POST',obj, function (d, bool) {
                        Util.hideLoding();
                        if(bool){
                ...
                /**
                 * @status 0编辑  1查看  2变更*/
                edtiPointData: function (obj, status,chargePointCode) {
                    Util.ajaxData('chargepoint/toview','POST',obj, function (d, bool) {
                        if(bool){
                            this.tplPoint(d.data, status,chargePointCode);
                        }
                    }.bind(this));
                },
                tplPoint: function (data, status,chargePointCode) {
                    var _this=this;
                ...
                addPointData: function (url, $box,chargePointCode, status) {
                    var data = this.pointForm($box);
                    if (this.required == 1) {
                        this.required = 0;
                        return false;
                    }
                    ...
                    if(chargePointCode){
                        data.chargePointCode = chargePointCode;
                    }
                    if (status) {
                        data.status = status;
                    }

                    Util.ajaxData(url,'POST',data, function (d, bool) {
                        Util.hideLoding();
                        if(bool){
                            window.location.reload();
                        }
                ...
                pointForm: function ($box) {
                    var obj = {}, alert = $box, _this = this, textarea = alert.find('textarea');
                    alert.find('input:text').each(function () {
                        if ($(this).val() == '') {
                            _this.required = 1;
                            $(this).parents('.a_box').addClass('errorTips');
                        } else {
                            $(this).parents('.a_box').removeClass('errorTips');
                        }
                        obj[$(this).attr('data-name')] = $(this).val();
                    });
                    if (textarea.val() == '') {
                        _this.required = 1;
                        textarea.parents('.a_box').addClass('errorTips');
                    } else {
                        textarea.parents('.a_box').removeClass('errorTips');
                        obj[textarea.attr('data-name')] = textarea.val();
                    }
                ...
                batchBtn: function () {
                    $('#batchSub').on('click', function () {
                        var data = this.batchSub();
                        if(data.chargePointCodes==''){
                            Util.tipsAlert(null,'请选择你要提交的计费点信息','tips_error');
                        }else{
                            Util.showLoding();
                            Util.ajaxData('chargepoint/batchPost','POST',data, function (d, bool) {
                                Util.hideLoding();
                                if(bool){
                                    window.location.reload();
                                }
                ...
                batchSub: function () {
                    ...
                    $('#itemContainer').find('input:checked').each(function () {
                        obj.chargePointCodes.push($(this).parents('tr').find('td').eq(2).text());
                    });
                    obj.chargePointCodes = obj.chargePointCodes.join(',');
                    return obj;
                },
                cloneAlert: function () {
...
require(
    ['jquery','entries/util/base','entries/service/point'],
    function ($,Base, Point) {
        $(function () {
            Base.init(function () {
                new Point();
...
define("entries/action/point.js", function(){});
