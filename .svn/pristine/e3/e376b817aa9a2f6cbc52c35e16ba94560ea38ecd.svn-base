 ...
 */
define(
    'entries/service/login',['jquery', 'Class', 'entries/util/util'],
    function ($, Class, Util) {
        var login = Class.create(
            {
                ...
                subBind: function () {
                    this.$submit.on('click', function () {
                        ...
                        Util.ajaxData('user/login', 'POST', data, function (d, bool) {
                            if (bool) {
                                if(this.url == 'channel'){
                                    Util.channelInit();
                                }else{
                                    window.location.href=this.url ? this.url : 'addservice.html';
                                }
                            }
                        }.bind(this));
                    }.bind(this));
                ...
                formVal: function () {
                    var data = {}, _this = this;
                    $('.logView').find('input').each(function () {
                        if ($(this).val() == '') {
                ...
                showAgteed: function () {
                    $('#sub_user_xy').on('click', function () {
                        Util.showAgreed();
                    });
                    Util.cloneAgreed();
                    Util.agreedBind($('.nextSub'),'cur');
                },
                tabBind: function () {
                    $('.title').find('.tab').on('click', function () {
                        if ($(this).index() == 0) {
                            $(this).addClass('cur').siblings('.cur').removeClass('cur');
                            $('.tab1').show().addClass('show_all');
                            $('.tab2').hide().removeClass('show_all');
                        } else {
                            $(this).addClass('cur').siblings('.cur').removeClass('cur');
                            $('.tab1').hide().removeClass('show_all');
                            $('.tab2').show().addClass('show_all');
                        }
                    });
                },
                getYzm: function () {
                    $('.reg_hq_yzm').on('click', function () {
                        var data = this.nextYzm();
                        if (!this.required) {
                            Util.hideLoding();
                            this.required = true;
                            return false;
                        ...
                        Util.showLoding();
                        Util.ajaxData('user/getValidateCode', 'POST', data, function (d, bool) {
                            Util.hideLoding();
                            if (bool) {
                                Util.tipsAlert(null, '已成功发送验证码，注意查收!', 'tips_success');
                            }
                        });
                    }.bind(this));
                },
                nextBind: function () {
                    $('#confirm').on('click', function () {
                        if (this.checked) {
                            $('.nextSub').addClass('cur');
                        } else {
                            $('.nextSub').removeClass('cur');
                        }
                    });
                    $('.nextSub').on('click', function () {
                        if ($('.nextSub').hasClass('cur')) {
                            var data = this.nextYzm(true);
                            if (!this.required) {
                                this.required = true;
                                return false;
                            }
                            Util.ajaxData('user/validateVerifyCode', 'POST', data, function (d, bool) {
                                if (bool) {
                                    window.location.href = 'setPassword.html?smsToken=' + d.data.smsToken + '&&userName=' + data.userName;
                                }
                            });
                        }
                    }.bind(this));
                },
                nextYzm: function (status) {
                    var $show = $('.show_all'), $yzm = $('.yzm'), data = {
                        bizType: 'REGISTER'
                    };
                    if ($show.find('input').val() == '') {
                        this.required = false;
                        $show.find('input').addClass('borRed');
                    } else {
                        $show.find('input').removeClass('borRed');
                        data.userName = $show.find('input').val();
                    }
                 ...
                 * 注册第二步，设置密码
                 */
                subPassNext: function () {
                    $('.setPass_next').on('click', function () {
                        var data = this.setPassPhone();
                        ...
                        Util.ajaxData('user/register', 'POST', data, function (d, bool) {
                            if (bool) {
                 ...
                 * 忘记密码
                 */
                forgetPassBind: function () {
                    this.forgetYzmBind();
                    this.forgetNext();
                    if (this.params.errorCode == 99) {
                        Util.tipsAlert(null, '验证码过期，请重新获取验证码!', 'tips_error');
                    }
                },
                forgetYzmBind: function () {
                    var $forget_zh = $('.forget_zh'), data = {
                        bizType: 'RESET_PASSWORD',
                        userName: ''
                    };
                    $('.forget_yzm').on('click', function () {
                        if ($forget_zh.val() != '') {
                            Util.showLoding();
                            data.userName = $forget_zh.val();
                            $forget_zh.removeClass('borRed');
                            Util.ajaxData('user/getValidateCode', 'POST', data, function (d, bool) {
                                Util.hideLoding();
                                if (bool) {
                                    Util.tipsAlert(null, '已成功发送验证码，注意查收!', 'tips_success');
                            ...
                            $forget_zh.addClass('borRed');
                        }
                    });
                },
                forgetNext: function () {
                    $('.forget_next').on('click', function () {
                        var data = this.formValForget();
                        if (!this.required) {
                            this.required = true;
                            return false;
                        }
                        Util.ajaxData('user/validateVerifyCode', 'POST', data, function (d, bool) {
                            if (bool) {
                                window.location.href = 'setPassPhone.html?smsToken=' + d.data.smsToken + '&&userName=' + data.userName;
                            }
                        });

                    }.bind(this));
                },
                formValForget: function () {
                    var _this = this, data = {
                        bizType: 'RESET_PASSWORD'
                    };
                    $('.logView').find('input').each(function () {
                 ...
                 * 忘记密码---手机或邮箱验证
                 */
                phoneVailBind: function () {
                    this.initPhone();
                },
                initPhone: function () {
                    $('.phone_hm').text(this.params.userName);
                },
                /**
                 * 忘记密码--——————设置密码
                 */
                setPhonePass: function () {
                    $('.setPassPhone_sub').on('click', function () {
                        var data = this.setPassPhone();
                        ...
                        Util.ajaxData('user/resetPassword', 'POST', data, function (d, bool) {
                            if (bool) {
                                Util.tipsAlert(null,'修改密码成功','tips_success', function () {
                ...
                setPassPhone: function () {
                    var data = {
                        resetPassword: '',
                        confirmPassword: '',
                        userName: this.params.userName,
                        verifyToken: this.params.smsToken
                    }, _this = this;
                    $('.logView').find('input').each(function (i) {
                        if ($(this).val().length >= 6) {
                            $(this).parent().removeClass('errorTips');
                            if (i == 0) {
                                data.resetPassword = $(this).val();
                            } else {
                                if (data.resetPassword === $(this).val()) {
                                    $(this).removeClass('borRed').parent().removeClass('errorTips');
                                    data.confirmPassword = $(this).val();
                                } else {
                                    $(this).addClass('borRed').parent().addClass('errorTips').find('.error').text("密码输入不一致");
                                    _this.required = false;
                                }
                            }
                        } else {
                            _this.required = false;
                            $(this).parent().addClass('errorTips').find('.error').text('请输入6到10位密码');
                        }
                ...
                editPassword: function () {
                    $('.resetPassword_sub').on('click', function () {
                        var data=this.resetFormVal();
                        ...
                        Util.ajaxData('user/modifyPassword','POST',data, function (d, bool) {
                            if(bool){
                                Util.tipsAlert(null,'修改密码成功','tips_success', function () {
                                    window.location.href='login.html';
                                });
                        ...
                        if($(this).val()!=''){
                            ...
                            if($(this).attr('data-name') != 'userName'){
                                if($(this).val().length >= 6){
                                    ...
                                    _this.required=false;
                                    $(this).parent().addClass('errorTips').find('.error').text('请输入6到20位密码!');
                                }
                            }else{
                                data[$(this).attr('data-name')]=$(this).val();
                            }
                            ...
                            _this.required=false;
                            $(this).parent().addClass('errorTips').find('.error').text('不能为空!');
                        }
                    });

                    if(data.newPassword != data.confirmPassword){
                        this.required=false;
                        $('.password_end').parent().addClass('errorTips').find('.error').text('密码输入不一致!');
                    }
                ...
                registeredSuccess:function(){
                    if($('#time_out').length > 0){
                        ...
                        var time=setInterval(function () {
                            $('#time_out').find('.ding').text(num);
                            if(num==1){
                                window.clearInterval(time);
                                window.location.href='/merchant/addservice.html';
                            }
                            num=num-1;
                        },1000);
                    }
...
require(
    ['jquery','entries/util/base','entries/service/login'],
    function ($,Base, Login) {
        $(function () {
            if($('.resetPassword_sub').length >0){
                Base.init(function () {
                    new Login();
                    $('#edit-userName').val(Poss.userName);
                });
            }else{
                new Login();
            }
...
define("entries/action/login.js", function(){});
