...
define(
    'entries/service/channelmanage-downloadgame',['jquery', 'Class', 'entries/util/util', 'template', 'pages'],
    function($, Class, Util, template) {
        var waiting = Class.create({
            ...
            resulttable: function() {
                this.datapost.pageNo = Number(this.pageNum)+1,
                this.datapost.pageSize = this.pageSize,
                this.datapost.gameName = $('.cns-text').val()

                Util.ajaxData('channel/pkgdl/gamelist2', 'POST', this.datapost, function(d, bool) {
                    if (bool) {
            ...
            pageBind: function() {
                ...
                Util.pageBind(_this.total, _this.pageSize, function(i) {
                    ...
                    $(".channel-game-list li").each(function() {
                        ...
                        $.each(_this.datagame.list, function(e, val) {
                            var gid = String(val).split(":")[1].split("}")[0];
            ...
            clickselected: function() {
                ...
                $(".channel-game-list").on("click", ".cg-in-content", function() {
                    ...
                    if ($(this).hasClass("selected")) {
                        ...
                        for (var i = 0; i < _this.dataUrl.length; i++) {
                            if (_this.dataUrl[i] == url) {
                                _this.dataUrl.remove(i);
                            }
                        }
                        console.log(_this.dataUrl);
                        _this.datagame.list.remove(index);
                        $(".allchecks").removeAttr("checked").removeClass("labelchecked");
                    } else {
                        _this.datagame.list.push("{gameId:" + gameid + "}");
                        _this.dataUrl.push(url);
                    ...
                    if ($(this).hasClass("labelchecked")) {
                        ...
                        if ($(".cg-in-content.selected").length < 10 && $(".cg-in-content").length > 0) {
                            ...
                            $(".channel-game-list li").each(function() {
                                var gameid = $(this).attr("gameid");
                                _this.datagame.list.push("{gameId:" + gameid + "}");
                                _this.dataUrl.push($(this).find('.download-link').attr('data-url'));
                            });
            ...
            applybug: function() {
                ...
                $("#applybug_btn").click(function() {
                    if (_this.datagame.list.length == 0) {
                        return false;
                    }
                    _this.eachDown(_this.dataUrl);
                });
            },
            eachDown: function(list) {
                console.log(list);
                list.map(function(d) {
                    window.open(d);
                });
            },
            showDownUrl: function() {
                $('.download-link').on('click', function() {
                    var url = $(this).attr('data-url');
                    Util.tipsAlert('下载地址', url, 'tips_text');
                })
            }
...
require(
    ['jquery','entries/util/base','entries/service/channelmanage-downloadgame'],
...
define("entries/action/channelmanage-subdownloadgame.js", function(){});
