...
public class MerchantAppAuditController extends BaseController{
	...
	public AjaxResult updateAppStatus(MerchantAppInfo app, HttpServletRequest request){
		//应用审批时需要选择业务类型:发行/联运(不需要申请计费点)
		if ((app.getAppCode().startsWith("D") || app.getAppCode().startsWith("A"))
				&& app.getStatus() == AppStatusEnum.access_finish.getStatus()
				&& app.getBizType() == null
				&& StringUtils.isBlank(app.getRemark())) {
			return AjaxResult.failed("手游需要选择业务类型");
		}
		...
		//设置审核时输入的业务类型bizType
		pastApp.setBizType(app.getBizType());
    ...
    public AjaxResult viewCallbackUrl(CallbackAudit callbackAudit ,HttpServletRequest request){
        try{
            ...
            if(callbackAudit.getId() != null){
				...
				data.put("appIcon", ca.getAppIcon());
    ...
    public AjaxResult callbackAudit(CallbackAudit callbackAudit , String handle) {
        ...
        try {
            ...
            if ("pass".equals(handle)) {
				...
				log.info("audit:{}", JSONObject.toJSONString(audit));
				//更新状态并修改游戏的回调地址
