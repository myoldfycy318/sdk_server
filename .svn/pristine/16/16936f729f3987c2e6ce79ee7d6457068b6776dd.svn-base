...
(function( factory ) {
	if ( typeof define === "function" && define.amd ) {
		...
		define('entries/jqinterface/jquery-ui.js',[ "jquery" ], factory );
	} else {
...
define(
    'entries/jqinterface/lightbullet.js',['jquery'],
    function($){
		(function($){
			...
			$.fn.dialogff = function(obj){
					...
					var ssclose = o.close;
					var sswidth = o.width;
					var ssheight = o.height;
					var sscloseendfunction=o.closeendfunction;
					...
					if(sstype=='open'){
						...
						self.appendTo("<div class='dialog-content' style='clear:both; display:none; position:fixed; width:100%; margin:0; padding:0; left:0; top:0; overflow-y:auto;'></div>").css({"display":"block","margin-top":100}).append("<div class='dialog-close'>X</div>").parent(".dialog-content").css({"height":$(window).height(),"z-index":zidx+1}).detach().appendTo("body").stop(true,true).fadeIn(300);
					...
					{
						self.find(".dialog-close").click();
					}

					$(window).resize(function(){
						$(".black-opacityff").css({"height":$(document).height()});
						$(".dialog-content").css({"height":$(window).height()});
					});
...
define(
    'entries/service/channelmanage-searchchannel',['jquery','Class','entries/util/util','template','pages','../jqinterface/jquery-ui.js','../jqinterface/lightbullet.js'],
        ...
        var waiting=Class.create(
            {
                ...
                downExeal:function(){
                    $('#donw-exeal').on('click',function(){
                        var url = Poss.channelUrl+'channel/export?domeAccessToken='+Util.cookie.dome_access_token;
                        window.open(url);
                    });
                },
                resulttable:function(){
                    ...
                    datapost={
                        pageNo: this.datapost.pageNo,
                        pageSize: this.datapost.pageSize
                    }

                   Util.ajaxData('channel/list', 'POST',this.datapost ,function (d, bool) {
                        if (bool) {
                ...
                pagenumclick:function(){
                    var _this = this;
                    $("body").on("click",".channel-numbtn",function(){
                        if(Number($("#channel_numtext1").val()) > Math.ceil(_this.total/_this.pageSize)){
                            return false;
                        }

                        _this.pageNum=Number($("#channel_numtext1").val())-1;
                        _this.datapost.pageNo=Number($("#channel_numtext1").val());

                        if(!isNaN(_this.pageNum)&&_this.pageNum >= 0){
                            _this.resulttable();
                        }
                    });
                },
                ...
                datacontrol:function(){
                    var _this = this;
                    $("#channel_firsttime").datepicker(Util.timeOptions);
                    $("#channel_lasttime").datepicker(Util.timeOptions);

                    $("#channel_searchbutton").click(function(){
                        var firsttime=parseInt($("#channel_firsttime").val().replace(/-/g,""));
                        var lasttime=parseInt($("#channel_lasttime").val().replace(/-/g,""));

                        if(firsttime>lasttime){
                            Util.tipsAlert('提示信息','起始时间必须小于最终时间','tips_error');
                        }
                        else{
                           
                            _this.datapost.name=$("#channel_role").val();
                            _this.datapost.beginDate=$("#channel_firsttime").val();
                            _this.datapost.endDate=$("#channel_lasttime").val();
                            _this.datapost.pageNo=1;
                            _this.pageNum=0;
                            _this.resulttable();
                        }
                ...
                seelinkdialog:function(){
                    var _this=this;
                    $(".channel-table").on("click",".see-link",function(event){
                        $("#search_dialog").dialogff({type:'open',close:'in',width:'410px'});

                        var thisclcode=$(this).parents("tr").find('.cl-c-code').text();

                        _this.datapost2={
                            "channelCode":thisclcode
                        }


                        Util.ajaxData('channel/query', 'POST',_this.datapost2 ,function (d, bool) {
                            if (bool) {
                                $("#cl_name").html(d.data.name);
                                $("#cl_email").html(d.data.email);
                                $("#cl_phone").html(d.data.mobilePhone);
                                $("#cl_contact").html(d.data.contact);
                                $("#cl_address").html(d.data.address);

                                if(d.data.cooperType==1){
                                    $("#cl_cptype").html("CPA");
                                }
                                else if(d.data.cooperType==2){
                                    $("#cl_cptype").html("CPS");
                                }
                            }
                        });
...
require(
    ['jquery','entries/util/base','entries/service/channelmanage-searchchannel'],
...
define("entries/action/channelmanage-subsearchchannel.js", function(){});
