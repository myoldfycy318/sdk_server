...
/**
 * util/api/merchantAppAPI
 */

define('entries/util/api/merchantAppAPI',[
  'jquery',
  'entries/util/util'
], function (
  $,
  ...
  var fetchList = function(query, cb) {
    util.ajaxData('merchantapp/v1/applist', 'GET', query, cb);
  }

  var fetchView = function(query, cb) {
    util.ajaxData('merchantapp/toview', 'GET', query, cb);
  }

  var deleteItem = function(query, cb) {
    util.ajaxData('merchantapp/dodel', 'POST', query, cb);
  }

  var updateState = function(query, cb) {
    util.ajaxData('merchantapp/updateState', 'POST', query, cb);
  }

  var save = function(data, cb) {
    util.ajaxData('merchantapp/save', 'POST', data, cb);
  }

  var applyModifyCallbackUrl = function(data, cb) {
    util.ajaxData('merchantapp/applyModifyCallbackUrl', 'POST', data, cb);
  }

  var toViewCallbackUrl = function(query, cb) {
    util.ajaxData('merchantapp/toViewCallbackUrl', 'GET', query, cb);
  }
  ...
  return {
    ...
    applyModifyCallbackUrl: applyModifyCallbackUrl,
    toViewCallbackUrl: toViewCallbackUrl
  }
});
/**
 * util/api/merchantAPI
 */

define('entries/util/api/merchantAPI',[
  'jquery',
  'entries/util/util'
], function (
  $,
  ...
  var fetchView = function(query, cb) {
    util.ajaxData('merchant/toview', 'GET', query, cb);
  }
...
define('entries/service/webgameAppList',[
    'jquery',
    'Class',
    'entries/util/util',
    'template',
    'entries/util/api/merchantAppAPI',
    'entries/util/api/merchantAPI',
    ...
    var app = Class.create({
        ...
        init: function (opts) {
            ...
            console.log('init webgameAppList');
            this.bindEvents();
            this.buildTable();
            this.bookDetail();
        },
        ...
        searchBind: function () {
            ...
            this.searchBtn.on('click', function () {
                obj[this.searchBox.find("input").attr('data-name')] = this.searchBox.find("input").val();
                this.buildTable(obj, 1);
            }.bind(this));
        },

        deleteBtnBind: function () {
            var that = this;
        ...
        anClick: function () {
            $(".an_click").on("click", function () {
                $(this).toggleClass("an_cur").parent().next().toggle();
            });
        },

        applyJr: function () {
            var that = this;
            $('.apply_jr').on('click', function () {
                var html, el = '.alert_box';
                that.appCode = $(this).attr('data-appCode');
                that.apply_status = $(this).attr('data-value');
                if (that.apply_status === "11") {
                    html = template('joinupTpl', {});
                    $('.alert-Box').css('height','260px');
                    $(el + ' .alert-Box').html('').html(html);
                    $(el).show();
                    that.bindJoinUpEvent();
                } else {
                    that.editAppStatus();
                }
        ...
        callbackBind: function () {
            var that = this;
            $('.update_callback').on('click', function () {
                var html, cb, appCode;

                appCode = $(this).attr('data-appCode');
                that.appCode = appCode;

                cb = function (d, bool) {
                    if (bool) {
                        html = template('joinupTpl', {
                            update: true,
                            payCallBackUrl: d.data.payCallbackUrl,
                            loginCallBackUrl: d.data.loginCallbackUrl,
                            appIcon: d.data.appIcon
                        });
                        $('.alert-Box').css('height','500px');
                        $('.alert_box .alert-Box').html('').html(html);
                        $('.alert_box').show();
                        that.bindJoinUpEvent();
                    }
                }
                merchantAppAPI.toViewCallbackUrl({
                    appCode: appCode
                }, cb);
        ...
        buildTable: function (obj, status) {
            ...
            cb = function (d, bool) {
                if (bool) {
                    ...
                    d.data.sizeTotal = Util.pageEnd(Number(d.data.appList.length), Number(this.pageNum), this.pageSize);

                    for (var i = 0; i < d.data.appList.length; i++) {
                        d.data.appList[i].encodeURI = encodeURI(d.data.appList[i].appName);
                    }

                    var html = template('tpl_table', d.data),
                        pageCon = template('tpl_pageCon', d.data);
                    if (status) {
                        this.status = 0;
                    }
                    this.tableTpl(html, pageCon);
                }
            }.bind(this)
            merchantAppAPI.fetchList(obj, cb);
        },

        bookDetail: function () {
            var cb = function (d, bool) {
                if (bool) {
                    var html = template('tpl_book_detail', d.data);
                    $('#view_detail').prepend(html);
                }
            };

            merchantAPI.fetchView({}, cb);
        },
        ...
        delApp: function (appCode) {
            merchantAppAPI.deleteItem({appCode: appCode}, function (d, bool) {
                if (bool) {
                    Util.tipsAlert(null, '应用删除成功', 'tips_success');
                    setTimeout(function () {
        ...
        editAppStatus: function (obj) {
            ...
            obj.appCode = this.appCode;
            obj.status = this.apply_status;
            ...
            cb = function (d, bool) {
                if (bool) {
                    Util.tipsAlert(null, msg + '成功', 'tips_success');
            ...
            Util.showLoding();
            merchantAppAPI.updateState(obj, cb);
        },

        callbackUpdate: function (obj) {
            if (!obj) {
                obj = {};
            }
            ...
            obj.appCode = this.appCode;
            Util.showLoding();
            merchantAppAPI.applyModifyCallbackUrl(obj, function (d, bool) {
                if (bool) {
                    Util.tipsAlert(null, '修改回调地址成功', 'tips_success');
        ...
        tableTpl: function (html, pageCon) {
            $('#tbody').empty().append(html);
            $('.pageCon').empty().append(pageCon);
        ...
        formHandle: function () {
            var items = ['loginCallBackUrl', 'payCallBackUrl'],
                selectorPrefix = '.app_zf .'
            ;
        ...
        fileLoad: function (item) {
                ...
                if (response.responseCode == 1000) {
                    if ($(item).hasClass('btn_file_je')) {
                        ...
                        $(img).find('.btn-del').on('click', function () {
                            _this.imgRemove($(this));
                        }).end().appendTo($(item).parents('.boxInput').find('.img_box'));

                        hideInput.val(val);
                        if ($(item).parents('.fileBox').find('.btn-del').length >= 9) {
                            $(item).hide();
                        }
                    } else {
            ...
            /*    uploader.on('error', function (file) {
                    ...
                    if(file == "Q_EXCEED_NUM_LIMIT"){
                        Util.tipsAlert(null,'您上传的图片数量超过了上限9，请重新上传!','tips_error');
                    }
                });*/
        }
        /* form handle end */
...
/**
 * webgameAppList Action
 */

require(
  ['jquery','entries/util/base','entries/service/webgameAppList'],
  function ($,Base, App) {
    $(function () {
      Base.init(function () {
        new App();
...
define("entries/action/webgameAppList.js", function(){});
