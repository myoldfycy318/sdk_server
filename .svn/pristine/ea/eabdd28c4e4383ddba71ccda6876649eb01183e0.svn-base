...
import com.dome.sdkserver.constants.*;
...
//		}
	 ...
	 */
	@Override
	public boolean needChargePoint(String appCode) {
		Integer bizType = merchantAppMapper.selectAppBizType(appCode);
		log.info("appCode:{}, bizType:{}", appCode, bizType);
		//老数据bizType 为空.
		if(bizType != null){
			if(bizType == AppBizTypeEnum.联运.getValue()){
				log.info("是否需要计费点:false"); 
				return false;//不需要计费点
			}else {
				log.info("是否需要计费点:true");
				return true;
			}
		}else {
			//老数据都当做发行处理,都需要计费点
