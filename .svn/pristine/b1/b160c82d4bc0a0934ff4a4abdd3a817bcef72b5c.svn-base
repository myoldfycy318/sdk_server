...
public class WebController {
    ...
    public String pay(CpPayEntity cpPayEntity, String webpayKey, int totalFee, Model model, HttpServletRequest req) {
        ...
        try {
            ...
            if (ChannelEnum.webGameIndependentChannel.contains(cpPayEntity.getChannelCode())) {//页游独立渠道
                cpPayEntity.setGameName(appInfoEntity.getAppName());
                StringBuilder payWeb = new StringBuilder("webgame.pay.page.").append(cpPayEntity.getChannelCode());
                String webPayPage = domainConfig.getString(payWeb.toString());
                if (StringUtils.isBlank(webPayPage)) {
                    model.addAttribute("err", "页游独立平台支付页面配置错误");
                    return "/msg";
                }
                StringBuilder sb = new StringBuilder("redirect:" + webPayPage).append("?").append(cpPayEntity.obj2String());
                return sb.toString();
            }
            //说明没有金额信息，跳转金额选择页面
