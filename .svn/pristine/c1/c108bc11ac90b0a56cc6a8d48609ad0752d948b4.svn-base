 ...
 */
define(
    'entries/service/review',['jquery', 'Class','template','entries/util/util'],
        ...
        var review = Class.create(
            {
                ...
                temp_success: function () {
                    Util.ajaxData('merchant/toview','GET',{}, function (d, bool) {
                        if(bool){
                            if(d.data.status==2){
                                var html=template('tpl_tips_success',d.data);
                                $('#tpl_con').append(html);
                            }else if(d.data.status==3){
                                var html=template('tpl_tips_error',d.data);
                                $('#tpl_con').append(html);
                            }
                        }
                    });
                },
                tpl_success: function () {
                    $('#tpl_con')
                }
            }
        );
...
require(
    ['jquery','entries/util/base','entries/service/review'],
    function ($,Base, Review) {
        $(function () {
            Base.init(function () {
                new Review();
            });
...
define("entries/action/review.js", function(){});
