...
define(
    'entries/service/channelmanage-downloadapp',['jquery','Class','entries/util/util'],
    function ($,Class,Util) {
        var waiting=Class.create(
            {
                ...
                /**
                *@status 1 2 3 显示下载页面，其余显示申请页面
                */
                isApply:function(){
                   Util.ajaxData('channel/pkgdl/applyresult', 'POST',{} ,function (d, bool) {
                        if (bool) {
                            if(d.data.status == '1' || d.data.status == '2' || d.data.status == '3'){
                                $('#chan-down').show();
                                if(d.data.status == '2'){
                                    $('#message-app').empty().text(d.data.message);
                                    $('#chan-down').find('.applchannelbag').css('background','#eee');
                                }else if(d.data.status == '3'){
                                    $('#message-app').empty().text(d.data.message);
                                    $('#chan-down').find('.applchannelbag').css('background','#eee');
                                }else{
                                    $('#chan-down').find('.applchannelbag').attr('href',d.data.downloadUrl);  
                                }
                            }else{
                                $('#chan-apply').show();
                                this.subApply();
                ...
                subApply:function(){
                    $('#chan-apply').find('.applchannelbag').on('click',function(){
                       Util.showLoding();
                       Util.ajaxData('channel/pkgdl/appapply', 'POST',{} ,function (d, bool) {
                            Util.hideLoding();
                            if (bool) {
                                Util.tipsAlert(null,'申请成功,请等待后台审批!','tips_success');
                            }else{
                                Util.tipsAlert(null,'申请失败,请重试!','tips_error');
                            }
                        });                            
                    })
...
require(
    ['jquery','entries/util/base','entries/service/channelmanage-downloadapp'],
...
define("entries/action/channelmanage-subdownloadapp.js", function(){});
